...
class Recorder(System_Plugin_Base):
    ...
    def __init__(
        ...
        self.queue_info = win32com.client.Dispatch("MSMQ.MSMQQueueInfo")
    ...
    def receive_messages(self):
        self.queue_info.FormatName = f'direct=os:{self.computer_name}\\PRIVATE$\\{self.queue_name}'
        queue = None

        try:
            queue = self.queue_info.Open(1, 0)

            while True:
                msg = queue.Receive()
                logger.info(f'Got Message from {self.queue_name}: {msg.Label} - {msg.Body}')
                if msg.Label == 'Start':
                    if self.running:
                        logger.error('Pupil lab is already recording, please stop it and resync EEG and pupil lab.')
                    else:
                        self.toggle()
                else:
                    if self.running:
                        ...
                        logger.error('Pupil lab is already stopping recording, please resync EEG and pupil lab.')
